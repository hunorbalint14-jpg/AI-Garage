...
-- 1. Retired ORG slugs (mirrors location_slug_history). When a platform admin
-- renames an org's slug (its subdomain), the old slug is recorded so the proxy
-- 308-redirects old links and the slug is permanently reserved (enforced in app
-- code via findSlugConflict()).
...
-- bypasses RLS. Platform admins may also read it directly. (is_platform_admin
-- lives in the `private` schema since 20260612160000.)
drop policy if exists "org_slug_history_admin_read" on public.org_slug_history;
create policy "org_slug_history_admin_read"
  on public.org_slug_history for select
  using (private.is_platform_admin());

-- 2. uptime_checks: the synthetic probe now targets the ORG subdomain. Add
-- organization_id (location_id stays for legacy rows / non-tenant kinds and is
-- no longer written for tenant probes).
...
-- 3. platform_tenant_health: one row per ORGANISATION. Tenant samples are keyed
-- by target_key = the subdomain slug, which is now the org slug, so latest/agg
-- join on organizations.slug. Adds location_count + a branches jsonb for the
-- expandable roster row.
-- CREATE OR REPLACE cannot rename an existing view's columns (the live view
-- leads with location_id; this one leads with organization_id), so drop first.
drop view if exists public.platform_tenant_health;
create view public.platform_tenant_health
