...
import { AigMark } from "@/components/ui/aig-loader";

// In-app navigation progress, shared by both portals. Unlike a route
// `loading.tsx` (which unmounts the current page and shows a fallback), this
// keeps the current page mounted and lays a translucent dim + the animated
// brand "AI" mark *over* it while the next route loads — so the app stays
...
// Translucent dim + the centred animated brand "AI" mark. Rendered once, near
// the shell root, under a NavProgressProvider.
export function NavProgressOverlay() {
  ...
  return (
    <div
      role="status"
      aria-label="Loading"
      className="fixed inset-0 z-[9999] flex items-center justify-center bg-[#070b12]/55 backdrop-blur-[3px]"
    >
      <AigMark />
    </div>
  );
