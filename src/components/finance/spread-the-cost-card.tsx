...
// "Spread the cost" card — Bumper PayLater. Used on the token-gated quote page
// and the auth-gated portal invoice page; the only difference is which server
// action raises the application, so the caller binds it and passes it in as
// `start`. The customer enters the address Bumper needs for its soft credit
// check (we don't keep addresses on file and don't persist them), then gets
// sent to Bumper's hosted checkout.
...
type Props = {
  start: (address: FinanceAddressInput) => Promise<StartFinanceResult>;
  primaryColor: string;
  totalFormatted: string;
};

export function SpreadTheCostCard({ start, primaryColor, totalFormatted }: Props) {
  ...
  function handleStart() {
    ...
    startTransition(async () => {
      const result = await start({ buildingNumber, street, town, postcode });
