...
import { quickBooksAuthUrl } from "@/lib/accounting/quickbooks-provider";
...
// Step 1 of QuickBooks OAuth — staff (owner/admin) hits this and we
// redirect to Intuit's consent screen. Same signed-state pattern as the
// Xero flow: the callback lands on the apex domain where the session
// cookie is invisible, so the state token is the trust anchor.
export async function GET(request: NextRequest) {
  ...
  return NextResponse.redirect(quickBooksAuthUrl(stateToken));
}
