...
export async function GET(
  ...
  const [invoiceRes, orgRes] = await Promise.all([
    admin
      .from("invoices")
      .select("id, invoice_number, status, subtotal, vat_rate, vat_amount, total, discount_amount, discount_description, issued_at, due_at, paid_at, notes, location_id, job_id, customer:customers(full_name, email, phone)")
      .eq("id", id)
      .maybeSingle(),
    admin
      .from("organizations")
