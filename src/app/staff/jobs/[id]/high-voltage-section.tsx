  ...
  return (
    <section className="rounded-lg border p-4 flex flex-col gap-2">
      <div className="flex items-center justify-between gap-3">
        ...
        >
          <span className={`pointer-events-none inline-block h-4 w-4 rounded-full bg-ws-card shadow-sm transition-transform ${highVoltage ? "translate-x-4" : "translate-x-0"}`} />
        </button>
      </div>

      {highVoltage && warning.kind === "no_qualified_techs" && (
        <p className="rounded-md border border-ws-red-border bg-ws-red-bg px-3 py-2 text-xs text-ws-red">
