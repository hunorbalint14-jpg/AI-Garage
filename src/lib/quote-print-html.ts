// Print-friendly A4 quote HTML. Used by both the staff print route
// (src/app/api/quote/[id]/print/route.ts) and the customer print route
// (src/app/quote/[slug]/print/route.ts) so both render an identical,
// professional document for Save-as-PDF — same mechanism as
// invoice-print-html.ts. Pure: no auth, no DB — callers prepare the data.

export type QuotePrintData = {
  slug: string | null;
  status: string;
  title: string | null;
  customer_message: string | null;
  subtotal: number;
  vat_rate: number;
  vat_amount: number;
  total: number;
  created_at: string;
  sent_at: string | null;
  expires_at: string | null;
  revision_number: number;
  revision_note: string | null;
  deposit_required: boolean;
  deposit_pct: number | null;
  deposit_amount: number | null;
  customer: { full_name: string | null; email: string | null; phone: string | null } | null;
  vehicle: { registration: string | null; make: string | null; model: string | null; year: number | null } | null;
};

export type QuotePrintItem = {
  description: string;
  type: string;
  quantity: number;
  unit_price: number;
};

export type QuotePrintOrg = {
  name: string;
  logo_url: string | null;
  primary_color: string | null;
};

function fmt(n: number): string {
  return new Intl.NumberFormat("en-GB", { style: "currency", currency: "GBP" }).format(n);
}

function fmtDate(d: string): string {
  return new Date(d).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" });
}

// Escape DB/user-controlled strings before interpolating into raw HTML so a
// stray <, &, or " can't break the markup or inject content.
function esc(s: string): string {
  return s
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;");
}

export function buildQuotePrintHtml(args: {
  quote: QuotePrintData;
  items: QuotePrintItem[];
  org: QuotePrintOrg;
  /** Right-aligned contact line under "QUOTE" (e.g. phone · email). */
  contactLine?: string | null;
  /** Auto-open the browser print dialog on load (default true). */
  autoPrint?: boolean;
}): string {
  const { quote, items, org } = args;
  const autoPrint = args.autoPrint ?? true;
  const brandColor = org.primary_color ?? "#1f2937";
  const garageName = org.name;

  const itemRows = items
    .map(
      (it) => `
      <tr>
        <td class="desc">${esc(it.description)}</td>
        <td class="type">${esc(it.type)}</td>
        <td class="num">${it.quantity}</td>
        <td class="num">${fmt(it.unit_price)}</td>
        <td class="num bold">${fmt(it.quantity * it.unit_price)}</td>
      </tr>`,
    )
    .join("");

  const logoHtml = org.logo_url
    ? `<img src="${esc(org.logo_url)}" alt="${esc(garageName)}" style="max-height:60px;max-width:180px;object-fit:contain">`
    : `<span style="font-size:22px;font-weight:700;color:${brandColor}">${esc(garageName)}</span>`;

  const contactLine = args.contactLine ? esc(args.contactLine) : "";
  const issued = quote.sent_at ?? quote.created_at;
  const vehicleLine = quote.vehicle
    ? [
        quote.vehicle.registration,
        [quote.vehicle.year, quote.vehicle.make, quote.vehicle.model].filter(Boolean).join(" ") || null,
      ]
        .filter(Boolean)
        .join(" — ")
    : null;

  const revisionHtml =
    quote.revision_number > 1
      ? `<div class="revision">
      <div class="revision-title">Revised quote — version ${quote.revision_number}</div>
      ${quote.revision_note ? `<div class="revision-note">${esc(quote.revision_note)}</div>` : ""}
    </div>`
      : "";

  const depositHtml =
    quote.deposit_required && quote.deposit_amount
      ? `<p class="deposit">A deposit of ${fmt(Number(quote.deposit_amount))}${quote.deposit_pct ? ` (${quote.deposit_pct}%)` : ""} is required to confirm this work.</p>`
      : "";

  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Quote ${esc(quote.slug ?? "")} — ${esc(garageName)}</title>
<link rel="icon" href="/brand/icon/aigarage-favicon.svg" type="image/svg+xml">
<link rel="icon" href="/brand/icon/png/favicon-32.png" sizes="32x32" type="image/png">
<link rel="apple-touch-icon" href="/brand/icon/png/apple-touch-icon.png" sizes="180x180">
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif; font-size: 13px; color: #111827; background: #fff; padding: 40px 48px; max-width: 800px; margin: 0 auto; }
  .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 40px; }
  .header-right { text-align: right; }
  .quote-label { font-size: 28px; font-weight: 800; color: ${brandColor}; letter-spacing: -0.5px; }
  .quote-number { font-size: 13px; color: #6b7280; margin-top: 4px; }
  .contact { font-size: 12px; color: #6b7280; margin-top: 6px; }
  .quote-title { font-size: 18px; font-weight: 700; margin-bottom: 16px; }
  .revision { border: 1px solid #fcd34d; background: #fffbeb; border-radius: 6px; padding: 12px 14px; margin-bottom: 20px; }
  .revision-title { font-size: 12px; font-weight: 700; color: #92400e; }
  .revision-note { font-size: 12px; color: #b45309; margin-top: 4px; white-space: pre-wrap; }
  .meta { display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; margin-bottom: 32px; border-top: 2px solid ${brandColor}; border-bottom: 1px solid #e5e7eb; padding: 16px 0; }
  .meta-label { font-size: 10px; text-transform: uppercase; letter-spacing: .06em; color: #9ca3af; margin-bottom: 4px; }
  .meta-value { font-size: 13px; font-weight: 600; }
  .meta-sub { font-size: 12px; color: #6b7280; }
  .message { font-size: 12px; color: #374151; white-space: pre-wrap; margin-bottom: 24px; }
  table { width: 100%; border-collapse: collapse; margin-bottom: 24px; }
  thead th { background: #f9fafb; padding: 10px 12px; text-align: left; font-size: 10px; text-transform: uppercase; letter-spacing: .06em; color: #6b7280; border-bottom: 1px solid #e5e7eb; }
  tbody td { padding: 9px 12px; border-bottom: 1px solid #f3f4f6; vertical-align: top; }
  tbody tr:last-child td { border-bottom: 2px solid #e5e7eb; }
  .type { color: #6b7280; text-transform: capitalize; }
  .num { text-align: right; font-variant-numeric: tabular-nums; }
  .bold { font-weight: 600; }
  tfoot td { padding: 8px 12px; }
  tfoot tr:last-child td { font-size: 15px; font-weight: 700; padding-top: 12px; border-top: 2px solid #111827; }
  .deposit { font-size: 12px; color: #374151; margin-bottom: 24px; }
  .footer { margin-top: 48px; padding-top: 16px; border-top: 1px solid #e5e7eb; font-size: 11px; color: #9ca3af; }
  @media print {
    body { padding: 20px 28px; }
    @page { margin: 1cm; size: A4; }
  }
</style>
</head>
<body>
<div class="header">
  <div>${logoHtml}</div>
  <div class="header-right">
    <div class="quote-label">QUOTE</div>
    ${quote.slug ? `<div class="quote-number">${esc(quote.slug)}</div>` : ""}
    ${contactLine ? `<div class="contact">${contactLine}</div>` : ""}
  </div>
</div>

${quote.title ? `<div class="quote-title">${esc(quote.title)}</div>` : ""}
${revisionHtml}

<div class="meta">
  <div>
    <div class="meta-label">Prepared for</div>
    <div class="meta-value">${esc(quote.customer?.full_name ?? "—")}</div>
    ${quote.customer?.email ? `<div class="meta-sub">${esc(quote.customer.email)}</div>` : ""}
    ${quote.customer?.phone ? `<div class="meta-sub">${esc(quote.customer.phone)}</div>` : ""}
    ${vehicleLine ? `<div class="meta-sub">${esc(vehicleLine)}</div>` : ""}
  </div>
  <div>
    <div class="meta-label">Issued</div>
    <div class="meta-value">${fmtDate(issued)}</div>
  </div>
  <div>
    <div class="meta-label">Valid until</div>
    <div class="meta-value">${quote.expires_at ? fmtDate(quote.expires_at) : "—"}</div>
    <div class="meta-sub" style="text-transform:capitalize">${esc(quote.status.replace(/_/g, " "))}</div>
  </div>
</div>

${quote.customer_message ? `<p class="message">${esc(quote.customer_message)}</p>` : ""}

<table>
  <thead>
    <tr>
      <th>Description</th>
      <th>Type</th>
      <th style="text-align:right">Qty</th>
      <th style="text-align:right">Unit price</th>
      <th style="text-align:right">Total</th>
    </tr>
  </thead>
  <tbody>${itemRows}</tbody>
  <tfoot>
    <tr>
      <td colspan="4" style="text-align:right;color:#6b7280">Subtotal</td>
      <td class="num">${fmt(quote.subtotal)}</td>
    </tr>
    <tr>
      <td colspan="4" style="text-align:right;color:#6b7280">VAT (${quote.vat_rate}%)</td>
      <td class="num">${fmt(quote.vat_amount)}</td>
    </tr>
    <tr>
      <td colspan="4" style="text-align:right">Total</td>
      <td class="num">${fmt(quote.total)}</td>
    </tr>
  </tfoot>
</table>

${depositHtml}

<div class="footer">${quote.expires_at ? `This quote is valid until ${fmtDate(quote.expires_at)}. ` : ""}Prices include VAT at ${quote.vat_rate}%. ${esc(garageName)} · Generated by AI Garage</div>
${autoPrint ? `<script>window.onload = () => window.print();</script>` : ""}
</body>
</html>`;
}
